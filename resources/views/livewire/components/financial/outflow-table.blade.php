            ...
            <tbody>
                @forelse($outflows as $outflow)
                    <tr>
                        ...
                        <td class="font-bold font-mono text-error">
                            <div class="flex items-center gap-1">
                                R$ {{ number_format(((float) $outflow->amount + (float) $outflow->adjustment_amount) - (float) $outflow->tax_amount, 2, ',', '.') }}
                                @if ($outflow->adjustment_amount != 0)
                                    <div class="tooltip tooltip-left"
                                        data-tip="Ajuste: R$ {{ number_format($outflow->adjustment_amount, 2, ',', '.') }} - {{ $outflow->adjustment_reason }}">
