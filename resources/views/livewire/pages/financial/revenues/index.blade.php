<div class="p-6">
    <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            ...
            <button wire:click="create" class="btn btn-primary">Nova Receita</button>
        </div>
        ...
        <div class="bg-base-100 rounded-box shadow overflow-x-auto">
            <table class="table">
                ...
                <tbody>
                            ...
                            <td>
                                @if ($revenue->paid_at)
                                    <span class="badge badge-success">Pago em
                                        {{ $revenue->paid_at->format('d/m/Y') }}</span>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-8 opacity-50">Nenhuma receita registrada.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
                            ...
                            <select wire:model.live="service_id" class="select select-bordered w-full">
                                ...
                                @foreach ($services as $s)
                                    <option value="{{ $s->id }}">#{{ $s->id }} - {{ $s->client->name }}
                                        (R$ {{ number_format($s->total, 2) }})</option>
                                @endforeach
                            </select>
                        </div>
